...
import { LogOut, User, Briefcase, Car } from 'lucide-react';

// Import components
import PassengerDashboard from './assets/PassengerDashboard';
import DriverDashboard from './assets/DriverDashboard';
import LocationAutocomplete from './assets/LocationAutocomplete';
...
const UberFriendsApp = () => {
  ...
  const handleSignup = async () => {
    ...
    if (activeTab === 'driver') {
      if (!vehicleId || !driverLocation || !contactNumber) {
        return alert("Drivers need Vehicle ID, Location and Contact Number.");
      }
    }
    ...
    const payload = { 
      ...
      location: driverLocation ? driverLocation.address : ''
    };
            ...
            <div className="space-y-4">
              ...
              {mode === 'signup' && (
                <div className="animate-slide-in space-y-4">
                  <input 
                    className="w-full bg-gray-100 p-3 rounded-lg outline-none focus:ring-2 focus:ring-black transition" 
                  ...
                  {activeTab === 'driver' && (
                    <>
                      <input 
                        className="w-full bg-gray-100 p-3 rounded-lg outline-none focus:ring-2 focus:ring-black transition" 
                        placeholder="Vehicle ID (e.g. KA-01-AB-1234)" 
                        value={vehicleId} 
                      ...
                      <LocationAutocomplete
                        ...
                        onLocationSelect={setDriverLocation}
                        type="pickup"
                      />
                    </>
                  )}
              ...
              {mode === 'login' ? (
                ...
                <button 
                  ...
                  className="w-full bg-green-600 text-white py-3 rounded-lg font-bold text-lg hover:bg-green-700 transition transform active:scale-95"
                >
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* Header */}
      <div className="bg-black text-white p-4 sticky top-0 z-50 shadow-md">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="bg-white text-black p-1.5 rounded font-bold text-xs tracking-tighter">
              UF
            </div>
            <h1 className="text-lg font-bold tracking-tight">
              Uber-Friends 
              <span className="font-normal text-gray-400 text-xs ml-2 border-l border-gray-600 pl-2 uppercase tracking-widest">
                {user.role} Portal
              </span>
            </h1>
          </div>
          <div className="flex items-center gap-6">
            ...
            <button 
              ...
              className="bg-gray-800 p-2 rounded-full hover:bg-red-600 transition flex items-center gap-2 px-4"
            >
              <LogOut size={16}/> 
      ...
      <div className="fixed top-24 right-4 z-50 space-y-2 w-80 pointer-events-none">
        {notifications.map((n, i) => (
          <div 
            key={i} 
            className="bg-blue-600 text-white p-4 rounded-lg shadow-xl animate-slide-in pointer-events-auto flex items-start gap-3"
          >
            <div className="mt-1">
              <Car size={16}/>
            </div>
            <div>
              <p className="font-bold text-xs uppercase mb-1 opacity-80">
                {n.type}
              </p>
